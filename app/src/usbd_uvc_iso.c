...
LOG_MODULE_REGISTER(usbd_uvc_iso, CONFIG_USBD_VIDEO_LOG_LEVEL);

#define UVC_VBUF_DONE 1
/* +14: 10 statische VC+VS-Eintraege ([0..9]) + 2 Platzhalter (if1_alt1, if1_ep) +
 * 1 NULL + 1 Reserve. Runtime-Format-Deskriptoren starten bei [10] und
 * ueberschreiben die Platzhalter; if1_alt1/if1_ep werden am Ende re-inseriert. */
#define UVC_MAX_FS_DESC (CONFIG_USBD_VIDEO_MAX_FORMATS + 14)
#define UVC_MAX_HS_DESC (CONFIG_USBD_VIDEO_MAX_FORMATS + 14)
...
struct uvc_desc {
	...
	/* VS Alt 0: Idle (bNumEndpoints=0), enthaelt Class-Specific Deskriptoren */
	...
	/* VS Alt 1: Streaming (bNumEndpoints=1), ISO Endpoint */
...
/* ISO HS High-Bandwidth: 1024 * 3 Transaktionen = 3072 Bytes/Mikroframe */
#define UVC_ISO_MPS_HS 3072U
UDC_BUF_POOL_VAR_DEFINE(uvc_buf_pool, UVC_TOTAL_BUFS, UVC_TOTAL_BUFS * UVC_ISO_MPS_HS,
...
static size_t uvc_get_bulk_mps(struct usbd_class_data *const c_data)
{
	...
	/* ISO HS High-Bandwidth: 1024 * 3 = 3072 Bytes/Mikroframe */
	if (USBD_SUPPORTS_HIGH_SPEED &&
	    usbd_bus_speed(uds_ctx) == USBD_SPEED_HS) {
		return UVC_ISO_MPS_HS;
	}

	/* ISO FS: 1023 Bytes/Frame */
	return 1023U;
}
				     ...
				     const uint8_t request)
{
	...
	/* ISO HS High-BW: dwMaxPayloadTransferSize = MPS = 3 * 1024 = 3072 Bytes/Mikroframe.
	 * NICHT frame_size + header — der Host erwartet hier die maximale Groesse
	 * einer einzelnen ISO-Payload (inklusive UVC-Header), also == MPS. */
	uint32_t max_payload_size = (uint32_t)uvc_get_bulk_mps(cfg->c_data);
			      ...
			      const struct uvc_control_map **const map)
{
	...
	if (ifnum == cfg->desc->if1_alt0.bInterfaceNumber) {
		switch (selector) {
...
static int uvc_init(struct usbd_class_data *const c_data)
{
	...
	/* DEBUG: Zustand VOR uvc_assign_desc() = was init_configuration_inst() sah */
	LOG_INF("uvc_init BEGIN: hs_desc_idx=%u", data->hs_desc_idx);
	for (int _j = 0; _j < UVC_MAX_HS_DESC; _j++) {
		struct usb_desc_header *_h = cfg->hs_desc[_j];

		if (_h == NULL) {
			LOG_INF("  PRE[%d] = NULL", _j);
			break;
		}
		if (_h->bLength == 0) {
			LOG_INF("  PRE[%d] = nil_desc (terminator)", _j);
			break;
		}
		const uint8_t *_r = (const uint8_t *)_h;

		LOG_INF("  PRE[%d]: len=%u type=0x%02x [2]=0x%02x [3]=0x%02x",
			_j, _r[0], _r[1], _r[2], _r[3]);
	}
	...
	/* ISO Alt 1 Interface Deskriptor vor dem Endpoint einfuegen */
	ret = uvc_assign_desc(dev, &cfg->desc->if1_alt1, true, true);
	if (ret != 0) {
		return ret;
	}
	...
	/* DEBUG: EP-Deskriptoren nach assign_ep_addr() — prueft ob wMaxPacketSize unveraendert */
	LOG_INF("uvc_init: if1_ep_hs addr=0x%02x bmAttr=0x%02x wMPS=0x%04x interval=%u",
		cfg->desc->if1_ep_hs.bEndpointAddress,
		cfg->desc->if1_ep_hs.bmAttributes,
		sys_le16_to_cpu(cfg->desc->if1_ep_hs.wMaxPacketSize),
		cfg->desc->if1_ep_hs.bInterval);
	LOG_INF("uvc_init: if1_ep_fs addr=0x%02x bmAttr=0x%02x wMPS=0x%04x interval=%u",
		cfg->desc->if1_ep_fs.bEndpointAddress,
		cfg->desc->if1_ep_fs.bmAttributes,
		sys_le16_to_cpu(cfg->desc->if1_ep_fs.wMaxPacketSize),
		cfg->desc->if1_ep_fs.bInterval);
	LOG_INF("uvc_init: if1_alt1 iface=%u alt=%u nEP=%u",
		cfg->desc->if1_alt1.bInterfaceNumber,
		cfg->desc->if1_alt1.bAlternateSetting,
		cfg->desc->if1_alt1.bNumEndpoints);
	...
	/* DEBUG: hs_desc[] Dump nach uvc_init() — zeigt was init_configuration_inst() sah */
	LOG_INF("uvc_init: hs_desc_idx=%u fs_desc_idx=%u", data->hs_desc_idx, data->fs_desc_idx);
	for (int _i = 0; _i < UVC_MAX_HS_DESC; _i++) {
		struct usb_desc_header *_h = cfg->hs_desc[_i];

		if (_h == NULL) {
			LOG_INF("  hs_desc[%d] = NULL (hard terminator)", _i);
			break;
		}
		if (_h->bLength == 0) {
			LOG_INF("  hs_desc[%d] = nil_desc (bLength=0, soft terminator)", _i);
			break;
		}
		const uint8_t *_r = (const uint8_t *)_h;

		LOG_INF("  hs_desc[%d]: len=%3u type=0x%02x [2]=0x%02x [3]=0x%02x",
			_i, _r[0], _r[1], _r[2], _r[3]);
	}
...
void uvc_set_video_dev(const struct device *const dev, const struct device *const video_dev)
{
	...
	cfg->desc->if0_hdr.baInterfaceNr[0] = cfg->desc->if1_alt0.bInterfaceNumber;
					     ...
					     size_t *const next_vbuf_offset)
{
	...
	bool _last_transfer = (fmt->pitch == 0)
		? (*next_vbuf_offset >= vbuf->bytesused)
		: (*next_line_offset >= fmt->height);
	if (_last_transfer) {
					     ...
					     size_t *const next_vbuf_offset)
{
	...
	struct video_format *fmt = &data->video_fmt;
	struct net_buf *buf;
	/* ISO: jeder Transfer maximal MPS Bytes (3072 fuer HS High-BW).
	 * Vorher stand hier 0x96000 (614400) — das hat uint16_t im net_buf
	 * zum Ueberlaufen gebracht (614400 mod 65536 = 24576 = 8 Mikroframes)
	 * und ist nicht ISO-konform: jedes usbd_ep_enqueue() = ein Mikroframe. */
	const size_t buf_len = MIN(uvc_get_bulk_mps(cfg->c_data),
				   vbuf->bytesused - data->vbuf_offset);
...
static int uvc_flush_vbuf(const struct device *dev, struct video_buffer *const vbuf)
{
	...
	if (next_vbuf_offset == vbuf->bytesused) {
		bi->vbuf = vbuf;
		/* ISO: kein ZLP — ISO-EP sendet immer genau buf->len Bytes.
		 * ZLP ist ein Bulk-Konzept (Signalisierung Ende eines Transfers).
		 * Bei ISO wuerde ZLP einen leeren Mikroframe erzeugen. */
		bi->udc.zlp = false;
	}
		       ...
		       const uint8_t alternate)
{
	...
	LOG_INF("uvc_update: iface=%u alt=%u", iface, alternate);

	if (alternate == 1U) {
		LOG_INF("UVC: Streaming START (Alt 1 — ISO EP aktiv)");
		/* Option D: ISO EP 0x82 manuell aktivieren falls Framework es
		 * nicht getan hat (ep_check_config() / high_bandwidth cap).
		 * udc_ep_enable() liefert -EALREADY wenn bereits aktiv — OK. */
		{
			struct usbd_context *uds_ctx = usbd_class_get_ctx(c_data);
			int ep_ret = udc_ep_enable(uds_ctx->dev,
						   0x82U,    /* EP addr: IN EP 2 */
						   0x05U,    /* bmAttributes: ISO asynchronous */
						   5120U,    /* wMaxPacketSize: 3x1024 HB */
						   1U);      /* bInterval: every microframe */
			if (ep_ret && ep_ret != -EALREADY) {
				LOG_ERR("uvc_update: ISO EP 0x82 enable failed: %d", ep_ret);
			} else {
				LOG_INF("uvc_update: ISO EP 0x82 ok (ret=%d)", ep_ret);
			}
		}
		/* ISO EP ist jetzt aktiv — ausstehende Video-Buffer senden.
		 * Ohne diesen Call bekommt der Host niemals Daten, weil
		 * uvc_flush_queue() nach SET_INTERFACE(1) nicht automatisch
		 * getriggert wird. */
		uvc_flush_queue(dev);
	} else if (alternate == 0U) {
		LOG_INF("UVC: Streaming STOP (Alt 0 — Idle)");
	}
}
...
static struct uvc_desc uvc_desc_##n = {						\
	...
	.if0_ot = {								\
		...
		.iTerminal = 0,							\
										...
										\
	/* VS Alt 0: Idle — enthaelt Class-Specific Deskriptoren, kein Endpoint */	\
	.if1_alt0 = {								\
		.bLength = sizeof(struct usb_if_descriptor),			\
		.bDescriptorType = USB_DESC_INTERFACE,				\
		.bInterfaceNumber = 1,						\
	...
	.if1_color = {								\
		...
		.bMatrixCoefficients = UVC_COLOR_BT601,				\
										...
										\
	/* VS Alt 1: Streaming — ein ISO Endpoint */			\
	.if1_alt1 = {								\
		.bLength = sizeof(struct usb_if_descriptor),			\
		.bDescriptorType = USB_DESC_INTERFACE,				\
		.bInterfaceNumber = 1,						\
		...
		.iInterface = 0,						\
										...
										\
	/* ISO FS: 1023 Bytes, bmAttributes=0x05 (ISO Asynchronous) */		\
	.if1_ep_fs = {								\
		.bLength = sizeof(struct usb_ep_descriptor),			\
		.bDescriptorType = USB_DESC_ENDPOINT,				\
		.bEndpointAddress = 0x82,					\
		.bmAttributes = 0x05,						\
		.wMaxPacketSize = sys_cpu_to_le16(1023),			\
		.bInterval = 1,							\
	},									\
										\
	/* ISO HS High-Bandwidth: 0x1400 = bits[10:0]=1024, bits[12:11]=10b (3×1024) */\
	.if1_ep_hs = {								\
		.bLength = sizeof(struct usb_ep_descriptor),			\
		.bDescriptorType = USB_DESC_ENDPOINT,				\
		.bEndpointAddress = 0x82,					\
		.bmAttributes = 0x05,						\
		.wMaxPacketSize = sys_cpu_to_le16(0x1400),			\
		.bInterval = 1,							\
	},									\
};										\
										\
struct usb_desc_header *uvc_fs_desc_##n[UVC_MAX_FS_DESC] = {			\
	...
	(struct usb_desc_header *) &uvc_desc_##n.if1_alt0,			\
	(struct usb_desc_header *) &uvc_desc_##n.if1_hdr,			\
	/* [10] if1_alt1: Platzhalter fuer init_configuration_inst(),		\
	 * wird zur Laufzeit durch Format-Deskriptoren ueberschrieben. */	\
	(struct usb_desc_header *) &uvc_desc_##n.if1_alt1,			\
	(struct usb_desc_header *) &uvc_desc_##n.if1_ep_fs,			\
	(struct usb_desc_header *) NULL,					\
};										\
										\
/* HS descriptor array — identisches Prinzip wie FS. */				\
