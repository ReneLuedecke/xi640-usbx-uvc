...
#include <cmsis_compiler.h>  /* __DSB(), __ISB() — Memory Barrier vor DMA-Start */
...
/*                                                                        */
/**************************************************************************/
/* Aus xi640_dcd.c — zaehlt echte SetupStageCallback-Aufrufe (nicht RXFLVL). */
extern volatile uint32_t g_usb_setup_count;

/* DataInStage-Zaehler: EP0 IN Transfer Complete (Descriptor gesendet).
 * Falls DIN=0 bei STP>0: USBX hat keinen Transfer gestartet — Stack-Problem.
 * Falls DIN=STP: jedes SETUP hatte eine IN-Antwort — Descriptor-Inhalt pruefen. */
volatile uint32_t g_usb_datain_count;
...
{
    /* EP0 IN Transfer Complete — Descriptor wurde an Host gesendet. */
    if (epnum == 0U) {
        g_usb_datain_count++;
    }
...
/*                                                                        */
/**************************************************************************/
/* Xi640 Diagnose-Zaehler — zaehlt wie oft HAL_PCD_ResetCallback aufgerufen wird.
 * Exportiert fuer xi640_dcd.c via extern.
 * Falls RSTCB=0 bei RST>0: HAL ruft Callback nicht auf (PCD Handle Problem). */
volatile uint32_t g_reset_cb_count;
